...
export default function AnalysisScreen() {
  ...
  const generateHTML = (r: any) => {
    ...
    const listItems = (arr: string[]) => (arr || []).map((h: string) => `<li style="margin-bottom:6px;font-size:13px;color:#333">${h}</li>`).join('');
    ...
    const segments = (r.segments || []).filter((s: any) => s.name).map((s: any) => `
      <div style="background:#f8faff;border-radius:8px;padding:12px;margin-bottom:8px">
        ...
        <div style="margin-top:6px;display:flex;gap:8px;flex-wrap:wrap">
          ${s.revenue ? `<span style="background:#eef4ff;color:#0052ff;padding:2px 8px;border-radius:4px;font-size:11px">Rev: ${s.revenue}</span>` : ''}
          ${s.growth ? `<span style="background:#f0fdf4;color:#22c55e;padding:2px 8px;border-radius:4px;font-size:11px">Growth: ${s.growth}</span>` : ''}
...
<style>
  ...
  .score-num{font-size:72px;font-weight:900;color:${scoreColor};line-height:1}
  .section{margin-bottom:24px}
  .section-title{font-size:15px;font-weight:700;margin-bottom:12px;padding-bottom:6px;border-bottom:2px solid #f0f4ff;color:#0a0e1a}
...
<div class="page">
  ...
  <div class="score-card">
    ...
    <div class="score-num">${r.health_score}<span style="font-size:28px;color:#888">/100</span></div>
  ...
  <div class="section">
    ...
    <p style="font-size:13px;line-height:1.8;color:#333;margin:0">${r.executive_summary || ''}</p>
  </div>
  ...
  ${r.profitability?.analysis ? `<div class="section"><div class="section-title">💰 Profitability</div><p style="font-size:13px;line-height:1.8;color:#333;margin:0 0 10px">${r.profitability.analysis}</p>${r.profitability.key_cost_drivers?.length ? `<strong style="font-size:12px">Cost Drivers:</strong><ul style="margin-top:4px">${(r.profitability.key_cost_drivers || []).map((c: string) => `<li>${c}</li>`).join('')}</ul>` : ''}</div>` : ''}

  ${r.growth?.analysis ? `<div class="section"><div class="section-title">📈 Growth</div><p style="font-size:13px;line-height:1.8;color:#333;margin:0 0 10px">${r.growth.analysis}</p>${r.growth.guidance && r.growth.guidance !== 'N/A' ? `<div style="background:#f0f9ff;border-radius:8px;padding:10px"><strong>Guidance:</strong> ${r.growth.guidance}</div>` : ''}</div>` : ''}

  ${r.liquidity?.analysis ? `<div class="section"><div class="section-title">💧 Liquidity</div><p style="font-size:13px;line-height:1.8;color:#333;margin:0">${r.liquidity.analysis}</p></div>` : ''}

  ${r.debt?.analysis ? `<div class="section"><div class="section-title">🏦 Debt & Leverage</div><p style="font-size:13px;line-height:1.8;color:#333;margin:0">${r.debt.analysis}</p></div>` : ''}

  ${r.management_commentary ? `<div class="section"><div class="section-title">🎙️ Management Commentary</div>${r.management_commentary.overall_tone ? `<p style="margin:0 0 8px"><strong>Tone:</strong> ${r.management_commentary.overall_tone}</p>` : ''}${mgmtPoints ? `<strong style="font-size:12px">Key Points:</strong><ul style="margin-top:4px">${mgmtPoints}</ul>` : ''}${r.management_commentary.outlook_statement && r.management_commentary.outlook_statement !== 'N/A' ? `<div style="background:#f0f9ff;border-radius:8px;padding:10px;margin-top:8px"><strong>Outlook:</strong> ${r.management_commentary.outlook_statement}</div>` : ''}</div>` : ''}
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(uri, {
        ...
        dialogTitle: `${analysis.result.company_name || 'FinSight'} Analysis.pdf`,
        UTI: 'com.adobe.pdf'
      });
    } catch (e) {
  ...
  const handleShareTwitter = async () => {
    const r = analysis?.result;
    const text = encodeURIComponent(`📊 ${r?.company_name} Financial Analysis — Health Score: ${r?.health_score}/100 (${r?.health_label}) via @FinSightAI`);
    const url = encodeURIComponent(`https://finsight-vert.vercel.app/analysis/${id}`);
    setShowShareMenu(false);
  ...
  const handleShareNative = async () => {
    ...
    try {
      await Share.share({
        ...
        url: `https://finsight-vert.vercel.app/analysis/${id}`,
  ...
  const handleCopyLink = async () => {
    setShowShareMenu(false);
    const url = `https://finsight-vert.vercel.app/analysis/${id}`;
    try {
    ...
    <ThemeContext.Provider value={{ dark }}>
      <View style={{ flex: 1, backgroundColor: t.bg }}>
        <ScrollView style={[styles.container, { backgroundColor: t.bg }]} showsVerticalScrollIndicator={false}>
          ...
          {/* Top Bar — only back and theme toggle */}
          ...
          {/* Bottom action buttons — ONLY here */}
          <View style={[styles.bottomActions, { backgroundColor: t.card }]}>
            ...
            <View style={styles.shareRow}>
              ...
              <TouchableOpacity style={[styles.shareBtn, { backgroundColor: '#0A66C2' }]} onPress={handleShareNative}>
                <Text style={styles.shareBtnText}>📤 More</Text>
              </TouchableOpacity>
            </View>

            <TouchableOpacity style={[styles.copyLinkBtn, { borderColor: t.border, backgroundColor: t.cardAlt }]} onPress={handleCopyLink}>
              <Text style={[styles.copyLinkText, { color: t.text }]}>🔗 Copy Shareable Link</Text>
            </TouchableOpacity>
          </View>
        ...
        {/* Share Menu Overlay */}
        {showShareMenu && (
          <TouchableOpacity style={styles.overlay} onPress={() => setShowShareMenu(false)}>
            <View style={[styles.shareMenu, { backgroundColor: t.card }]}>
              <Text style={[styles.shareMenuTitle, { color: t.text }]}>Share Analysis</Text>
              {[
                { label: '💬 WhatsApp', action: handleShareWhatsApp, bg: '#25D366' },
                { label: '𝕏 Twitter / X', action: handleShareTwitter, bg: '#000' },
                { label: '📤 Other Apps', action: handleShareNative, bg: '#0052FF' },
                { label: '🔗 Copy Link', action: handleCopyLink, bg: t.cardAlt },
              ].map((opt, i) => (
                <TouchableOpacity key={i} style={[styles.shareMenuItem, { backgroundColor: opt.bg }]} onPress={opt.action}>
                  <Text style={[styles.shareMenuItemText, { color: opt.bg === t.cardAlt ? t.text : '#fff' }]}>{opt.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  downloadBtn: { borderRadius: 14, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12 },
  ...
  shareBtn: { flex: 1, borderRadius: 12, padding: 12, alignItems: 'center' },
  shareBtnText: { color: '#fff', fontSize: 12, fontWeight: '700' },
  copyLinkBtn: { borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1 },
  ...
  shareMenu: { borderRadius: 20, margin: 16, padding: 20 },
  shareMenuTitle: { fontSize: 16, fontWeight: '700', marginBottom: 14, textAlign: 'center' },
  shareMenuItem: { borderRadius: 12, padding: 14, marginBottom: 8, alignItems: 'center' },
  shareMenuItemText: { fontSize: 15, fontWeight: '700' },
});
